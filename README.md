...
本課題の制約は16時間だったが、実際には約20時間を要した。超過の主因は、最低要件を超えたスコープ拡張にある。

Organization/User モデルの導入、5段階ステータス（PointOut → Open → InProgress → Done → Confirmed）、ロール別権限マトリクス、StatusChangeLog による履歴追跡など、課題の最低要件には含まれない機能を設計・実装した。これらは「施工現場で実際に運用する」ことを想定した判断であり、3段階ステータス・認証なしの最小構成では実務上の欠陥を残すと考えたためである。

結果として設計の質は確保できたが、16時間という制約の中での優先順位判断としては最適ではなかった。再度取り組むならば、まず最低要件を3段階ステータス・認証なしの MVP として16時間以内に完成させ、拡張機能は後続イテレーションで段階的に追加する進め方が適切だったと考える。スコープを広げる判断自体は誤りではないが、時間制約下では「何を後回しにするか」の判断こそが PM の本質的な能力であり、その点で課題が残った。
