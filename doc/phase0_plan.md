...
| R12 | ロール別の画面遷移・表示範囲制御（Admin全権限/Supervisor支店内全体/Worker自担当のみ） | 拡張 |
...
※ ロール定義：
- **Admin（本部管理者）**: 支部CRUD、ユーザー管理、全支店・全プロジェクトの閲覧・指摘登録・プロジェクト登録/編集。本部に所属。
- **Supervisor（現場監督）**: 所属支店のプロジェクト全体を管理。指摘の登録（ReportedBy）、作業担当者への割り振り（Assignee設定）、是正完了の確認・否認。支部に所属。プロジェクト情報は閲覧のみ。
...
- 支店名の一覧を表示。支店の追加・編集・削除もこの画面から操作可能
- 支店を選択すると画面Bへ遷移

#### 画面B: 支店管理 / プロジェクト一覧（タブ切り替え）
...
- Supervisor: 同じレイアウトだが閲覧のみ（自分の所属支店に自動スコープ）
...
- フロアによるフィルタリング機能（プルダウン or タブ）
- ステータスによるフィルタリング機能
- Admin/Supervisor: 全指摘を表示、指摘の新規追加が可能
- Worker: 自分がAssigneeの指摘のみ表示、新規追加は不可
- 「3Dビューで表示」ボタン: 選択中のプロジェクトに紐づく建造物の3Dビューへ遷移（フロアフィルタの状態を引き継ぐ）
...
**フロアフィルタ:**
- 指摘一覧画面から引き継いだフロアフィルタの状態を維持
- 3Dビュー側でもフロアフィルタを変更可能（プルダウン）
- フロア変更時は3Dビュー（セクションボックス等）と指摘一覧パネルの両方が連動して更新
...
| Method | Path | 責務 | 種別 | 権限 |
|--------|------|------|------|------|
| GET | /api/organizations/{orgId}/users | 支店内ユーザー一覧取得 | Query | Admin |
...
| Method | Path | 責務 | 種別 | 権限 |
|--------|------|------|------|------|
| GET | /api/organizations/{orgId}/projects | 支店のプロジェクト一覧取得（進捗率・指摘件数含む） | Query | 認証済み※ |
...
### 判断14: フロア一覧画面の廃止と画面遷移の再設計
- **決定**: 独立した「フロア一覧」画面を廃止し、指摘一覧画面内のフロアフィルタに統合する。画面遷移を「プロジェクト選択 → 指摘一覧 → 3Dビュー → 指摘詳細」に再設計する。
- **理由**: 画面遷移の見直しにより「プロジェクト選択 → 指摘一覧」の直接遷移が自然な導線となった。フロアは指摘の属性の一つであり、独立画面を設ける必然性が薄い。フィルタとして組み込むことでユーザーの操作ステップを削減し、フロア切り替え時の画面遷移も不要になる。3Dビュー側ではフロアフィルタと連動したセクションボックス制御を維持する。指摘詳細も独立ページではなく画面遷移先として表示する。
...
- **決定**: Adminにも全支店・全プロジェクトに対する指摘登録権限を付与する（スーパー権限）。
- **理由**: Adminは全体管理者として各支店のプロジェクトを横断的に管理する。品質監査や緊急対応の観点から、Admin自身が指摘を登録できることは実運用上合理的。ReportedByにAdminのUserIdが入ることで「誰が登録したか」も追跡可能。
...
- **決定**: ロールに応じてログイン後の起点画面と表示スコープを分離する。Admin: 支店一覧 → 支店管理。Supervisor: 自分の所属支店のプロジェクト一覧に直接遷移。Worker: 自分が担当の指摘があるPJのみ表示。
- **理由**: 各ロールの業務実態に合わせた導線を提供することで、不要な情報ノイズを排除し操作効率を向上させる。Adminは横断的管理、Supervisorは支店内の現場管理、Workerは自分の作業にフォーカスするという責務の違いをUIレベルでも反映する。
